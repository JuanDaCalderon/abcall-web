...
describe('ViewIncidenciaComponent', () => {
  ...
  /*fit('should load incident on ngOnInit', () => {
    incidenciasService.getIncidenciaById.and.returnValue(of(mockIncidente));
    component.ngOnInit();
    expect(component.incidentForm.get('estado')?.value).toEqual('abierto');
  });*/
  ...
  it('should return new gestor when current gestor is senior', () => {
    ...
    //expect(newGestor[0]).toBe('6'); // '5' is the id of the senior gestor
    //expect(newGestor[1]).toBe('gestorled');
  ...
  /*it('should load incident on ngOnInit', () => {
    ...
    component.ngOnInit();
    expect(component.incidentForm.get('estado')?.value).toEqual('abierto');
    expect(component.incidentForm.get('cliente')?.value).toEqual('1');
    expect(component.incidentForm.get('nombreUsuario')?.value).toEqual('1');
    expect(component.incidentForm.get('correoUsuario')?.value).toEqual('prueba@prueba.com');
    expect(component.incidentForm.get('telefonoUsuario')?.value).toEqual('123456789');
    expect(component.incidentForm.get('direccionUsuario')?.value).toEqual('Test address');
    expect(component.incidentForm.get('descripcionProblema')?.value).toEqual('Test description');
    expect(component.incidentForm.get('tipoIncidencia')?.value).toEqual('incidencia');
    expect(component.incidentForm.get('canalIngreso')?.value).toEqual('web');
    expect(component.incidentForm.get('prioridad')?.value).toEqual('baja');
    expect(component.incidentForm.get('comentarios')?.value).toEqual('Test comments');
  });*/

  /*  fit('should handle error when loading incident', async () => {
    incidenciasService.getIncidenciaById.and.returnValue(throwError('Error'));
    await component.ngOnInit();
    expect(toastrService.error).toHaveBeenCalledWith('Error al cargar la incidencia', 'Error', {
      closeButton: true,
      timeOut: 3000,
      positionClass: 'toast-bottom-center'
    });
